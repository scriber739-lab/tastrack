...
function ManualDivisionEntry({ onSave }) {
  ...
  const handleSave = () => {
    ...
    onSave(entries, { bill, date, policy, source, outcome, summary });
    setBill(""); setDate(today()); setSource(""); setSummary(""); setVotes({}); setConsistency({});
  };
  ...
  return (
    <div>
      ...
      <div style={{marginTop:14,background:"#f8f9ff",borderRadius:10,padding:"12px 14px",fontSize:12,color:"#555",lineHeight:1.8}}>
        <strong>To use in TasTrack:</strong> Copy Member Data JSON → open <code>tasmania-tracker.jsx</code> → find <code>const SEED = [</code> → replace with this data. Scores recalculate automatically.
      </div>
    </div>
  );
}
  ...
  const handleAIImport = useCallback((extractedDivisions, meta) => {
    ...
    setData(prev => {
      ...
      for (const div of extractedDivisions) {
        const divId = uid();
        newDivisions.push({ id:divId, bill:div.bill, date:div.date||meta.documentDate, policy:div.policyArea, source:meta.documentSource||"Tasmania Hansard", outcome:div.outcome, summary:div.summary, entryCount:div.votes?.length||0 });
        for (const v of (div.votes||[])) {
          if (!v.memberId || v.vote === "absent") continue;
          if (!next[v.memberId]) continue;
          next[v.memberId] = {
            ...next[v.memberId],
            votes: [...next[v.memberId].votes, { id:divId+"_"+v.memberId, bill:div.bill, date:div.date||meta.documentDate, policyArea:div.policyArea, vote:v.vote, consistency:"unknown", source:meta.documentSource||"Tasmania Hansard", divisionOutcome:div.outcome }]
          };
          totalVotes++;
        }
      }
  ...
  const handleManualSave = useCallback((entries, meta) => {
    ...
    setData(prev => {
      ...
      for (const { memberId, entry } of entries) {
        ...
        next[memberId] = { ...next[memberId], votes: [...next[memberId].votes, entry] };
      }
      return next;
    });
