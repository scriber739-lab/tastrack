    ...
    try {
      ...
      if (type==="members") {
        ...
        for (const m of arr) {
          ...
          // Upsert member attendance
          await sb("members", { method:"PATCH", body:JSON.stringify({
  ...
  const handleAIImport = useCallback(async (extractedDivisions, meta) => {
    ...
    try {
      ...
      if (meta.membersPresent?.length > 0) {
        // We do a simple increment via RPC or just track it separately
        // For now, store in member table via upsert
        for (const memberId of meta.membersPresent) {
          await sb(`members?id=eq.${memberId}`, {
            method: "PATCH",
            body: JSON.stringify({ sitting_days_present: "sitting_days_present + 1", sitting_days_total: "sitting_days_total + 1" }),
            prefer: "",
          }).catch(()=>{}); // ignore if this member isn't in DB yet
        }
      }
