...
Still extract these — they are important bill outcomes. Set "divisionType": "voice" and leave "votes" as an empty array [].
Examples: second readings, third readings, and bill passages that say "resolved in the Affirmative" without a division.

WHAT TO EXTRACT:
- Extract ALL divisions (formal and voice) for significant bills and motions
...
When in doubt, ask: "If a politician voted FOR this, would a reasonable person say they SUPPORT the policy area?" If no, set inverse:true.`;
      ...
      if (!rawResp || rawResp.trim() === "") throw new Error(`Empty response from proxy (HTTP ${resp.status}). Check Vercel function logs and confirm ANTHROPIC_API_KEY is set correctly.`);
      let data;
                ...
                <div style={{flex:1}}>
                  ...
                  <div style={{display:"flex",gap:8,flexWrap:"wrap"}}>
                    ...
                    {div.divisionType==="voice" ? (
                      <span style={{fontSize:11,padding:"2px 8px",borderRadius:20,background:"#f0f4ff",color:"#1a3a6b",fontWeight:700}}>
                        🗣️ Voice vote — no individual records
                      </span>
                    ) : (
                      <>
...
export default function AdminApp() {
  ...
  const handleAIImport = useCallback((extractedDivisions, meta) => {
    ...
    setData(prev => {
      ...
      // ---- Attendance: increment sittingDaysPresent for each member listed as present ----
      const sittingDaysCount = (meta.sittingDays||[]).length || 1;
      if (meta.membersPresent && meta.membersPresent.length > 0) {
        for (const memberId of meta.membersPresent) {
          if (!next[memberId]) continue;
          const currentPresent = parseInt(next[memberId].sittingDaysPresent) || 0;
          const currentTotal   = parseInt(next[memberId].sittingDaysTotal)   || 0;
          next[memberId] = {
            ...next[memberId],
            sittingDaysPresent: String(currentPresent + sittingDaysCount),
            sittingDaysTotal:   String(currentTotal   + sittingDaysCount),
          };
        }
        // Members NOT in membersPresent still accumulate total sitting days (they were just absent)
        for (const m of ALL_MEMBERS) {
          if (meta.membersPresent.includes(m.id)) continue;
          if (!next[m.id]) continue;
          const currentTotal = parseInt(next[m.id].sittingDaysTotal) || 0;
          next[m.id] = { ...next[m.id], sittingDaysTotal: String(currentTotal + sittingDaysCount) };
        }
      }
      ...
      for (const div of extractedDivisions) {
        const divId = uid();
        newDivisions.push({ id:divId, bill:div.bill, date:div.date||meta.documentDate, policy:div.policyArea, source:meta.documentSource||"Tasmania Hansard", outcome:div.outcome, summary:div.summary, inverse:div.inverse||false, divisionType:div.divisionType||"formal", entryCount:div.votes?.length||0 });
    ...
    const attendanceNote = meta.membersPresent?.length > 0 ? ` · attendance updated for ${meta.membersPresent.length} members` : "";
    setDivisions(prev => [...prev, ...newDivisions]);
    showToast(`✅ Imported ${extractedDivisions.length} divisions · ${totalVotes} votes${attendanceNote}`);
    setTab("members");
  }, []);
