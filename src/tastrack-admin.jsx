...
// ============================================================
// SPEECH EXTRACTOR
// Upload a Hansard transcript — Claude extracts statements
// and detects potential contradictions
// ============================================================
function SpeechExtractor({ showToast }) {
  const [stage,     setStage]     = useState("idle");
  const [fileInfo,  setFileInfo]  = useState(null);
  const [extracted, setExtracted] = useState(null);
  const [error,     setError]     = useState(null);
  ...
  const handleFile = async (file) => {
    setError(null); setStage("reading");
    setFileInfo({ name:file.name, size:(file.size/1024).toFixed(1)+"KB" });
    setProgress("Reading transcript...");
    try {
      let content, mediaType;
      const ext = file.name.split(".").pop().toLowerCase();
      if (ext==="pdf" || file.type==="application/pdf") {
      ...
      setStage("extracting"); setProgress("Sending to Claude for speech analysis...");

      const memberList = ALL_MEMBERS.map(m=>`id:${m.id} — ${m.name} (${m.party}, ${m.electorate})`).join("\n");

      const systemPrompt = `You are extracting parliamentary speech data from a Tasmanian House of Assembly Hansard transcript.

The full member list:
${memberList}

YOUR TASK: Extract significant statements, promises, and policy positions made by members during debate. Also identify potential contradictions between what members say in this transcript vs known positions.

For each significant statement:
- It should be a clear policy position, promise, commitment, or factual claim
- It should be attributable to a specific member
- It should be substantive enough to track over time

Return ONLY a valid JSON object, no markdown, no preamble:
{
  "documentDate": "YYYY-MM-DD",
  "documentSource": "e.g. Hansard, 15 May 2025 — Second Reading of Housing Bill",
  "statements": [
    {
      "id": "short unique id",
      "memberId": 11,
      ...
      "policyArea": "one of: Housing & Rent, Environment & Climate, Health & Hospitals, Gambling Reform, Macquarie Point Stadium, Fiscal Policy & Debt, Governance & Accountability, Transport & Infrastructure, Education, Agriculture & Fishing, Other",
      "quote": "exact words from the transcript, max 100 words",
      "summary": "one sentence summary of the statement",
      "significance": "high|medium|low",
      "context": "brief context of why this was said"
    }
  ],
  "potentialContradictions": [
    {
      ...
      "statementA": {
        "date": "YYYY-MM-DD",
        "quote": "the statement from THIS transcript",
        "source": "Hansard reference",
        "context": "context"
      },
      "statementB": {
        "date": "YYYY-MM-DD or 'unknown'",
        "quote": "the earlier/conflicting statement if known from transcript, otherwise describe",
...
Only flag contradictions if there is clear evidence within the transcript itself. Do not speculate about contradictions with information outside this document.`;

      let messages;
      if (mediaType) {
        messages = [{ role:"user", content:[
          { type:"document", source:{ type:"base64", media_type:mediaType, data:content } },
          { type:"text", text:"Extract all significant statements and potential contradictions from this Hansard transcript. Return only valid JSON." }
        ]}];
      } else {
        messages = [{ role:"user", content:`Extract significant statements and contradictions from this Hansard transcript. Return only valid JSON, no preamble.\n\nTRANSCRIPT:\n${content.slice(0,40000)}` }];
      }
      ...
      if (!rawResp || rawResp.trim()==="") throw new Error(`Empty response from proxy (HTTP ${resp.status}).`);
      let data;
      try { data = JSON.parse(rawResp); } catch(e) { throw new Error(`Non-JSON response: ${rawResp.slice(0,200)}`); }
      ...
      if (fb!==-1&&lb!==-1&&lb>fb) clean=clean.slice(fb,lb+1);
      let parsed;
      try { parsed = JSON.parse(clean); } catch(e) { throw new Error("Unparseable JSON: "+raw.slice(0,200)); }

      if ((!parsed.statements||parsed.statements.length===0) && (!parsed.potentialContradictions||parsed.potentialContradictions.length===0)) {
        throw new Error("No statements or contradictions found. Make sure this is a Hansard transcript (not a Votes & Proceedings document).");
      }

      // Pre-select all
      const sel = {};
      (parsed.statements||[]).forEach(s=>sel["s_"+s.id]=true);
      (parsed.potentialContradictions||[]).forEach(c=>sel["c_"+c.id]=true);
      setSelected(sel);
      setExtracted(parsed);
      setStage("review");
      setProgress("");
  ...
  const handleSave = async () => {
    ...
    setSaving(true);
    try {
      const stmtsToSave = (extracted.statements||[]).filter(s=>selected["s_"+s.id]);
      const consToSave  = (extracted.potentialContradictions||[]).filter(c=>selected["c_"+c.id]);
      let savedStmts = 0, savedCons = 0;

      // Save statements to Supabase
      if (stmtsToSave.length > 0) {
        const rows = stmtsToSave.map(s=>({
          id:        s.id+"_"+s.memberId,
        ...
        await sb("statements?on_conflict=id", { method:"POST", body:JSON.stringify(rows), headers:{"Prefer":"resolution=ignore-duplicates,return=minimal"}, prefer:"" });
        savedStmts = rows.length;
      }

      // Save contradictions to Supabase
      if (consToSave.length > 0) {
        const rows = consToSave.map(c=>({
          id:          c.id+"_"+c.memberId,
        ...
        await sb("contradictions?on_conflict=id", { method:"POST", body:JSON.stringify(rows), headers:{"Prefer":"resolution=ignore-duplicates,return=minimal"}, prefer:"" });
        savedCons = rows.length;
      }

      showToast(`✅ Saved ${savedStmts} statements · ${savedCons} contradictions to Supabase`);
      setStage("done");
    } catch(e) {
  ...
  if (stage==="review" && extracted) {
    const stmts = extracted.statements||[];
    const cons  = extracted.potentialContradictions||[];
    ...
    return (
      <div>
        <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:14}}>
          <div>
            <div style={{fontWeight:700,fontSize:15,color:"#1a1a2e"}}>✅ Found {stmts.length} statements · {cons.length} potential contradictions</div>
            <div style={{fontSize:12,color:"#888",marginTop:2}}>{fileInfo?.name} · {extracted.documentDate}</div>
          </div>
          <Btn onClick={reset} outline color="#888" small>← New file</Btn>
        </div>
        ...
        {stmts.length > 0 && (
          <div style={{marginBottom:16}}>
            <div style={{fontWeight:700,fontSize:13,color:"#1a1a2e",marginBottom:8}}>💬 Statements & Promises</div>
            {stmts.map(s=>{
              const key = "s_"+s.id;
              const isSel = selected[key];
              const member = ALL_MEMBERS.find(m=>m.id===s.memberId);
              return (
                <div key={key} style={{marginBottom:8,borderRadius:10,border:`2px solid ${isSel?"#1a3a6b":"#ddd"}`,background:isSel?"#f8f9ff":"#fafafa",padding:"12px 14px",cursor:"pointer"}}
                  ...
                  <div style={{display:"flex",alignItems:"flex-start",gap:10}}>
                    ...
                    <div style={{flex:1}}>
                      <div style={{display:"flex",gap:6,flexWrap:"wrap",marginBottom:4}}>
                        <span style={{fontSize:11,fontWeight:700,color:PARTY_COLORS[member?.party]||"#555"}}>{s.memberName}</span>
                        <span style={{fontSize:10,padding:"1px 7px",borderRadius:20,background:"#f5f0ff",color:"#5c3a9e",fontWeight:700}}>{s.type.replace(/_/g," ")}</span>
                        <span style={{fontSize:10,padding:"1px 7px",borderRadius:20,background:s.significance==="high"?"#fbe8e8":s.significance==="medium"?"#fff7e0":"#f0f4ff",color:s.significance==="high"?"#9e2d2d":s.significance==="medium"?"#8a5f00":"#3a5a9e",fontWeight:700}}>{s.significance}</span>
                        <span style={{fontSize:10,padding:"1px 7px",borderRadius:20,background:"#f0f4ff",color:"#555",fontWeight:600}}>{s.policyArea}</span>
                      </div>
                      <div style={{fontSize:13,fontStyle:"italic",color:"#333",marginBottom:4}}>"{s.quote}"</div>
                      <div style={{fontSize:11,color:"#666"}}>{s.summary}</div>
                    </div>
                  </div>
                </div>
              );
        ...
        {cons.length > 0 && (
          <div style={{marginBottom:16}}>
            <div style={{fontWeight:700,fontSize:13,color:"#9e2d2d",marginBottom:8}}>⚡ Potential Contradictions</div>
            {cons.map(c=>{
              const key = "c_"+c.id;
              const isSel = selected[key];
              const member = ALL_MEMBERS.find(m=>m.id===c.memberId);
              const sevColor = c.severity==="high"?"#9e2d2d":c.severity==="medium"?"#8a5f00":"#3a5a9e";
              const sevBg    = c.severity==="high"?"#fbe8e8":c.severity==="medium"?"#fff7e0":"#f0f4ff";
              return (
                <div key={key} style={{marginBottom:8,borderRadius:10,border:`2px solid ${isSel?"#9e2d2d":"#ddd"}`,background:isSel?"#fff8f8":"#fafafa",padding:"12px 14px",cursor:"pointer"}}
                  onClick={()=>setSelected(p=>({...p,[key]:!p[key]}))}>
                  <div style={{display:"flex",alignItems:"flex-start",gap:10}}>
                    <div style={{width:20,height:20,borderRadius:5,border:`2px solid ${isSel?"#9e2d2d":"#ccc"}`,background:isSel?"#9e2d2d":"#fff",display:"flex",alignItems:"center",justifyContent:"center",flexShrink:0,marginTop:1}}>
                      {isSel&&<span style={{color:"#fff",fontSize:12,fontWeight:900}}>✓</span>}
                    </div>
                    <div style={{flex:1}}>
                      <div style={{display:"flex",gap:6,flexWrap:"wrap",marginBottom:4}}>
                        <span style={{fontSize:11,fontWeight:700,color:PARTY_COLORS[member?.party]||"#555"}}>{c.memberName}</span>
                        <span style={{fontSize:10,padding:"1px 7px",borderRadius:20,background:sevBg,color:sevColor,fontWeight:700}}>{c.severity.toUpperCase()}</span>
                        <span style={{fontSize:10,padding:"1px 7px",borderRadius:20,background:"#f0f0f0",color:"#555",fontWeight:700}}>{c.type.replace(/_/g," ")}</span>
                      </div>
                      <div style={{fontWeight:600,fontSize:13,color:"#222",marginBottom:3}}>{c.summary}</div>
                      <div style={{fontSize:11,color:"#666"}}>{c.explanation}</div>
                    </div>
                  </div>
                </div>
              );
        ...
        <div style={{display:"flex",gap:10,marginTop:4}}>
          <Btn onClick={handleSave} disabled={selCount===0||saving} color="#2d7a5f">
            {saving?"⏳ Saving…":`✅ Save ${selCount} item${selCount!==1?"s":""} to Supabase`}
          </Btn>
          <Btn onClick={reset} outline color="#888">Cancel</Btn>
        </div>
      </div>
  ...
  if (stage==="done") return (
    <div style={{textAlign:"center",padding:"30px 0"}}>
      <div style={{fontSize:40,marginBottom:10}}>✅</div>
      <div style={{fontWeight:700,fontSize:16,color:"#2d7a5f",marginBottom:6}}>Saved to Supabase</div>
      <div style={{fontSize:13,color:"#888",marginBottom:20}}>Statements and contradictions are live in the app immediately.</div>
      <Btn onClick={reset}>Extract another transcript</Btn>
    </div>
  );
  ...
  return (
    <div>
      <div style={{background:"#f0faf4",borderRadius:10,padding:"12px 14px",marginBottom:16,fontSize:12,color:"#2d5a3d",lineHeight:1.7,borderLeft:"3px solid #2d9e5f"}}>
        <strong>How this works:</strong> Download a Hansard transcript from{" "}
        <a href="https://search.parliament.tas.gov.au/adv/hahansard" target="_blank" rel="noreferrer" style={{color:"#2d7a5f"}}>Tasmania Hansard</a>,
        drop it here. Claude reads every speech and extracts: policy statements and promises (for Trust Score), and potential contradictions (for the Contradictions tab). Works with PDF or plain text.
      </div>
      <div style={{background:"#fff7e0",borderRadius:10,padding:"10px 14px",marginBottom:16,fontSize:12,color:"#7a5500",lineHeight:1.6,borderLeft:"3px solid #e0a020"}}>
        <strong>Note:</strong> Contradictions detected from a single transcript reflect only what is said within that document. Cross-transcript contradictions are detected by your Python scraper (<code>tastrack_scraper.py --mode speeches</code>).
      </div>
      <div onDrop={e=>{e.preventDefault();const f=e.dataTransfer.files[0];if(f)handleFile(f);}} onDragOver={e=>e.preventDefault()} onClick={()=>fileRef.current?.click()}
        style={{border:"2px dashed #c0d0e8",borderRadius:12,padding:"40px 20px",textAlign:"center",cursor:"pointer",background:"#f8f9ff"}}
        onMouseEnter={e=>{e.currentTarget.style.borderColor="#1a3a6b";e.currentTarget.style.background="#f0f4ff";}}
        ...
        <div style={{fontWeight:700,fontSize:15,color:"#1a3a6b",marginBottom:4}}>Drop a Hansard transcript here</div>
        <div style={{fontSize:12,color:"#888",marginBottom:12}}>or click to browse · PDF or TXT</div>
        <div style={{display:"inline-block",padding:"8px 20px",background:"#1a3a6b",color:"#fff",borderRadius:8,fontSize:13,fontWeight:700}}>Choose File</div>
      </div>
      {(stage==="reading"||stage==="extracting") && (
  ...
  return (
    <div style={{minHeight:"100vh",background:"#f0f2f7",fontFamily:"'Segoe UI',system-ui,sans-serif"}}>
      ...
      <div style={{maxWidth:900,margin:"0 auto",padding:"18px 14px"}}>
        ...
        {tab==="speeches" && (
          <div style={{background:"#fff",borderRadius:14,border:"1px solid #eee",padding:"18px",boxShadow:"0 2px 10px rgba(0,0,0,0.06)"}}>
            <h3 style={{fontFamily:"Georgia, serif",marginTop:0,marginBottom:14,fontSize:16}}>🎙️ Hansard Speech Extractor</h3>
            <SpeechExtractor showToast={showToast} />
          </div>
        )}
