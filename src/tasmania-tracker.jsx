...
const POLICIES = [
  ...
  },
  {
    ...
    divisions: [
      ...
      { id:"d_s2", bill:"Stadium Cost Disclosure Motion",                     date:"2025-03-11", outcome:"defeated", inverse:true, summary:"Motion to require full public disclosure of stadium cost projections. Voting FOR = opposed to stadium / pro-transparency." },
    ]
  },
  {
    key: "fiscal",
    ...
    divisions: [
      ...
      { id:"d_f2", bill:"Asset Privatisation Prevention Motion",        date:"2024-10-15", outcome:"defeated", inverse:true, summary:"Motion to prevent privatisation of state-owned assets. Voting FOR = opposed to privatisation / pro-public-ownership." },
    ]
  },
  {
    key: "governance",
              ...
              <div key={i} style={{marginBottom:10,paddingBottom:10,borderBottom:i<policy.divisions.length-1?"1px solid #f5f5f5":"none"}}>
                ...
                <div style={{marginTop:8,display:"flex",flexWrap:"wrap",gap:4}}>
                  {politicians.filter(p=>(p.votingRecord||[]).some(v=>v.divisionId===d.id)).map(p=>{
                    ...
                    const effectiveFor = d.inverse ? vr.vote!=="for" : vr.vote==="for";
                    return (
                      <span key={p.id} onClick={()=>onSelectPolitician(p)}
                        style={{fontSize:11,padding:"2px 8px",borderRadius:20,cursor:"pointer",fontWeight:600,
                          background:effectiveFor?"#e8f5e8":"#fbe8e8",
                          color:effectiveFor?"#2d7a2d":"#9e2d2d",
                          border:`1px solid ${effectiveFor?"#b8dfc0":"#f0bcbc"}`}}>
                        {p.name.split(" ")[1]||p.name.split(" ")[0]} {effectiveFor?"✓":"✗"}
                      </span>
                    );
                  })}
                </div>
